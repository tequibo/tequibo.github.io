...
let color_patterns=[
    ...
    ["#EF3E36","#17BEBB","#281A1F"],
    ["#161616","#FCF7F8","#8F0000"],
...
"NOISE","LIFE","FOOD","MAKE","SENSE","TODAY","NIGHT", "TEARS","CORRECT","PERFECT","FUN","JOKE","LOL","ME","THANKS","BYE","SMOKE","HIGH","SOON","WAIT"]
let eyes = ['˘','●','ȍ',' ﾟ','⍜','⌾','✿','๏','☉','❍','◕','─', '•','^','˘', '★','ʘ','▼','ಠ','°','*','＋','°','˘','✦','◉','◐','◑','♥','❛','ಡ','•ิ']
let mouths = ['‿', '‸','ヮ','ω','﹏','ε','､','ｪ','︵','╭╮','～','.', '.','─'];
let gl_dir;//p5.Vector.random2D()
class Brush{
    constructor(x,y, c, vel_dir, dev=PI/2, rot=PI*2,sc=1){
        this.scale=fxrand()*fxrand()*fxrand()*sc
        this.position=createVector(x,y);
        this.vel=vel_dir;//createVector(6*w/600,0);
    ...
    for(var i=0;i<am;i++){
        ...
        switch (brush_side) {
            case 0:
                xpos=0;
                ypos=i/am*h;
                vel_dir=createVector(6*w/600,0)
            break;        
            case 1:
                xpos=i/am*w;
                ypos=0;
                vel_dir=createVector(0,6*h/600)
            break;
            case 2:
                xpos=w;
                ypos=i/am*h;
            ...
            case 3:
                xpos=i/am*w;
                ypos=h;
                vel_dir=createVector(0,-6*w/600)
            break;
        }
        // let b = new Brush(p1.x+cos(a)*circle_r/3, p1.y+sin(a)*circle_r/3,fxrand()>.5?c1:c2,PI/2,0,.23)
        let b = new Brush(xpos, ypos,fxrand()>.5?c1:c2,vel_dir,PI/2,0,.33)
        b.o=i;
