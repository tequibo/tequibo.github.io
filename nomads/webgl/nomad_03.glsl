...
vec3 getColor(vec2 uv){    
    ...
    float angle =TAU*.25+TAU*u_param3;//u_mouse.x/u_resolution.x*TAU+TAU*.25;
    ...
    if(d_m.y==1.0){//head
        ...
        col=vec3(cos(p.y*11.+(u_time*1.)*TAU),
        cos(p.y*3.+(u_time*1.)*TAU),
        cos(p.y*5.+(u_time*3.)*TAU));
        ...
        if(p.z>-10.){
            ...
            po.x*=1.4;
            po2.y+=.6;
            po2.y+=sin(po2.x*9.0*u_param4-4.7)*0.1;//pow(po2.x,4.)*5.;
            float eyebrows = box((po+vec2(0.05,-0.4))*rotate2d(-TAU*.04),vec2(.2,0.05));
            float whites = length(po)-.3;//whites
            float iris = length(po)-.25*u_param1;//iris
            po = po*rotate2d(TAU*u_time*.5);
            iris = sdCross(po,vec2(0.2,0.06*u_param4),0.);
    ...
    if(d_m.y==2.0){
        ...
        col=vec3(cos(p.y*53.+sin(u_time*1.)*TAU), cos(p.y*53.+sin(u_time*2.)*TAU), cos(p.y*53.+sin(u_time*3.)*TAU));
