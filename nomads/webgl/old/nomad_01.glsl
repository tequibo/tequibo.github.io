...
vec2 map(vec3 point){
    ...
    vec2 leg = vec2(sdCone(leg_p,vec2(sin(a_leg), cos(a_leg)),1.),3.0);
    // body_p.xy=body_p.xy*rotate2d(TAU*.5);
    // vec2 body = vec2(sdRoundCone(body_p,0.3,0.19,.5),3.0);
    float a_body = TAU*.02;
    vec2 body = vec2(sdCone(body_p,vec2(sin(a_body), cos(a_body)),1.4)-.2,3.0);
...
vec3 getColor(vec2 uv){    
    ...
    if(d_m.y==1.0){
        // col=vec3(0.8784, 0.0706, 0.0706);
        col=vec3(p.y*2., 1.0706, 0.0706);
        col=oklab_mix(vec3(0.9059, 0.2314, 0.2314), vec3(0.2, 0.9412, 0.9922),p.y*3.+1.);
    ...
    if(d_m.y==2.0){
        ...
        col=oklab_mix(vec3(0.9686, 0.0039, 0.0039), vec3(0.8588, 0.3373, 0.3373),p.y*3.+.5);
